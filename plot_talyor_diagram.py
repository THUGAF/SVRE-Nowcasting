...
def plot_taylor_diagram(root: str, paths: List, models: List, target_path: str, std_range: List, std_num: int, colors: List):
    ...
    truth = torch.load(os.path.join(root, paths[-1], 'images', 'sample', 'truth', 'truth.pt'))
    ...
    for i, path in enumerate(paths):
        pred = torch.load(os.path.join(root, path, 'images', 'sample', 'pred', 'pred.pt'))
...
if __name__ == '__main__':
    ...
    plot_taylor_diagram('results', 
                        ['AttnUNet', 'AttnUNet_CV', 'AttnUNet_GAN_CV'], 
                        ['AGAN(G)', 'AGAN(GSVRE)', 'AGAN(GASVRE)'], 
                        'test/taylor_ablation.jpg', std_range=[5, 15], std_num=6, 
                        colors=[colors.colors[0], colors.colors[2], colors.colors[3]])
    plot_taylor_diagram('results',
                        ['PySTEPS', 'ConvLSTM_EF', 'SUNet', 'AttnUNet_GAN_CV'], 
                        ['PySTEPS', 'ConvLSTM-EF', 'SmaAt-UNet', 'AGAN(GASVRE)'],
                        'test/taylor_comparison.jpg', std_range=[5, 15], std_num=6, 
